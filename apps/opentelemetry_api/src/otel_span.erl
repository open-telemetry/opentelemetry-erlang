...
-type sdk_start_opts() :: #{attributes := opentelemetry:attributes_map(),
                            links := [opentelemetry:link()],
                            is_recording := boolean(),
                            start_time := opentelemetry:timestamp(),
                            kind := opentelemetry:span_kind()}.
...
-export_type([start_opts/0, sdk_start_opts/0]).

%% @doc Validates the start options for a span and fills in defaults.
-spec validate_start_opts(start_opts()) -> sdk_start_opts().
