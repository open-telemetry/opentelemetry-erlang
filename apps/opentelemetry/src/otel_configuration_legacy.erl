...
%%
%% Adapts legacy application and OS environment configuration to the concrete
%% runtime configuration consumed by the SDK. OS environment variables take
%% precedence over application environment values.
%% @private
%%%-------------------------------------------------------------------------
-module(otel_configuration_legacy).

-export([resolve/1,
         merge_list_with_environment/3,
         report_cb/1]).

-type resolved() :: otel_configuration:resolved().
...
-spec resolve(app_env()) -> resolved().
resolve(AppEnv) ->
    %% `undefined' is the legacy sentinel used while resolving the deprecated
    %% `register_loaded_applications' alias. general/2 replaces it with the
    %% canonical default when neither legacy setting is configured.
    Unresolved = (otel_configuration:resolve(#{}))#{create_application_tracers := undefined},
    Config = general(AppEnv, Unresolved),
    Config1 = span_limits(AppEnv, Config),
    Config2 = sampler(AppEnv, Config1),
    Config3 = processors(AppEnv, Config2),
    sweeper(AppEnv, Config3).

-spec span_limits(app_env(), resolved()) -> resolved().
span_limits(AppEnv, ConfigMap) ->
    merge_list_with_environment(config_mappings(span_limits), AppEnv, ConfigMap).

-spec general(app_env(), map()) -> resolved().
...
-spec sweeper(app_env(), resolved()) -> resolved().
...
-spec processors(app_env(), resolved()) -> resolved().
...
merge_processor_config(otel_batch_processor, Opts, ConfigMap, AppEnv) ->
    ...
    maps:merge(otel_configuration:span_processor_defaults(otel_batch_processor),
...
merge_processor_config(otel_simple_processor, Opts, ConfigMap, AppEnv) ->
    SimpleEnvMapping = [{ssp_exporting_timeout_ms, exporting_timeout_ms},
                        {traces_exporter, exporter}],
    maps:merge(otel_configuration:span_processor_defaults(otel_simple_processor),
...
-spec sampler(app_env(), resolved()) -> resolved().
...
transform(span_processor, batch) ->
    {otel_batch_processor,
     otel_configuration:span_processor_defaults(otel_batch_processor)};
transform(span_processor, simple) ->
    {otel_simple_processor,
     otel_configuration:span_processor_defaults(otel_simple_processor)};
