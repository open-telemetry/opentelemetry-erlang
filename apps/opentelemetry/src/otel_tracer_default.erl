...
start_span(Ctx, Tracer={_, #tracer{on_start_processors=Processors,
    ...
    io:format("[otel_tracer_default/start_span] ~p ~p ~p ~p ~p ~n", [Opts, Opts1, Ctx, Processors, InstrumentationLibrary]),
    SpanCtx = otel_span_ets:start_span(Ctx, Name, Opts1, Processors, InstrumentationLibrary),
    io:format("[otel_tracer_default/start_span] ets: ~p~n", [SpanCtx]),
