...
export(Tab, Resource, #state{protocol=http_protobuf,
    ...
    case otel_otlp_metrics:to_proto(Tab, Resource) of
        empty ->
            ok;
        ProtoMap ->
            Proto = opentelemetry_exporter_metrics_service_pb:encode_msg(ProtoMap,
            ...
            case uri_string:normalize(#{scheme => Scheme,
                ...
                {error, Type, Error} ->
                    ...
                    error;
                Address ->
                    %% eqwalizer:fixme not sure why NewHeaders is considered [{[number()], binary() | iolist()}]
                    case httpc:request(post, {Address, NewHeaders, "application/x-protobuf", NewProto},
                        ...
                        {ok, {{_, Code, _}, _, Message}} ->
                            ...
                            error;
                        {error, Reason} ->
                            ?LOG_INFO("client error exporting metrics ~p", [Reason]),
                            error
                    end
            end
    end;
...
merge_with_environment(Opts) ->
    ...
    Config = #{otlp_endpoint => undefined,
               otlp_metrics_endpoint => undefined,
               otlp_headers => undefined,
               otlp_metrics_headers => undefined,
               otlp_protocol => undefined,
               otlp_metrics_protocol => undefined,
               otlp_compression => undefined,
               otlp_metrics_compression => undefined,
    ...
    case maps:get(otlp_endpoint, AppOpts) of
        ...
        _ ->
            ...
            Opts2 = update_opts(otlp_metrics_protocol, protocol, http_protobuf, AppOpts, Opts1),
            ...
            DefaultMetricsEndpoints = endpoints_append_path(maps:get(endpoints, Opts3), maps:get(protocol, Opts3)),

            %% now set `endpoints' to either the default metrics value or the user configured value
            Opts4 = update_opts(otlp_metrics_endpoint,
                                endpoints,
                                DefaultMetricsEndpoints,
                                AppOpts,
                                Opts3#{endpoints => DefaultMetricsEndpoints},
                                fun maybe_to_list/1),

            Opts5 = update_opts(otlp_headers, headers, [], AppOpts, Opts4),
            Opts6 = update_opts(otlp_metrics_headers, headers, [], AppOpts, Opts5),

            Opts7 = update_opts(otlp_compression, compression, undefined, AppOpts, Opts6),
            Opts8 = update_opts(otlp_metrics_compression, compression, undefined, AppOpts, Opts7),
...
endpoints_append_path(Endpoints, http_protobuf) ->
    endpoints_append_path_(Endpoints, ?DEFAULT_METRICS_PATH);
endpoints_append_path(Endpoints, _) ->
