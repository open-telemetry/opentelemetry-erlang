...
merge_with_environment(Opts) ->
    ...
    case maps:get(otlp_endpoint, AppOpts) of
        ...
        _ ->
            ...
            Opts2 = update_opts(otlp_traces_protocol, protocol, http_protobuf, AppOpts, Opts1),
            ...
            DefaultTracesEndpoints = endpoints_append_path(maps:get(endpoints, Opts3), maps:get(protocol, Opts3)),

            %% now set `endpoints' to either the default traces value or the user configured value
            Opts4 = update_opts(otlp_traces_endpoint,
                                endpoints,
                                DefaultTracesEndpoints,
                                AppOpts,
                                Opts3#{endpoints => DefaultTracesEndpoints},
                                fun maybe_to_list/1),

            Opts5 = update_opts(otlp_headers, headers, [], AppOpts, Opts4),
            Opts6 = update_opts(otlp_traces_headers, headers, [], AppOpts, Opts5),
...
endpoints_append_path(Endpoints, http_protobuf) ->
    endpoints_append_path_(Endpoints, ?DEFAULT_TRACES_PATH);
